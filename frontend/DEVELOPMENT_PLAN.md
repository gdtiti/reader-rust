...
- [x] 点击书签 → 跳转到对应章节位置
- [x] 删除书签
- [ ] 对应 API：`getBookmarks`、`saveBookmark`、`deleteBookmark`、`deleteBookmarks`
...
- [x] 搜索结果列表（章节名 + 匹配文本片段）
- [x] 点击结果 → 跳转并高亮匹配文本
- [ ] 对应 API：如后端支持 `searchBookContent`
...
- [ ] 显示当前书在各书源下的信息
- [ ] 切换后重新加载章节列表和内容
- [ ] 对应 API：`setBookSource`、`getBookSourceByBook` 等
