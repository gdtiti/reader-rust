...
pub async fn get_book_groups(State(state): State<AppState>, Query(access_q): Query<AccessTokenQuery>) -> Result<Json<ApiResponse<Value>>, AppError> {
...
pub async fn save_book_group(State(state): State<AppState>, Query(access_q): Query<AccessTokenQuery>, Json(group): Json<BookGroup>) -> Result<Json<ApiResponse<Value>>, AppError> {
    let user_ns = state.user_service.resolve_user_ns(access_q.access_token.as_deref(), access_q.secure_key.as_deref()).await.map_err(|_| AppError::BadRequest("NEED_LOGIN".to_string()))?;
    state.book_group_service.save_group(&user_ns, group).await?;
    Ok(Json(ApiResponse::ok(serde_json::json!("success"))))
}

pub async fn delete_book_group(State(state): State<AppState>, Query(access_q): Query<AccessTokenQuery>, Json(param): Json<GroupIdParam>) -> Result<Json<ApiResponse<Value>>, AppError> {
    let user_ns = state.user_service.resolve_user_ns(access_q.access_token.as_deref(), access_q.secure_key.as_deref()).await.map_err(|_| AppError::BadRequest("NEED_LOGIN".to_string()))?;
...
pub async fn save_book_group_order(State(state): State<AppState>, Query(access_q): Query<AccessTokenQuery>, Json(groups): Json<Vec<BookGroup>>) -> Result<Json<ApiResponse<Value>>, AppError> {
    let user_ns = state.user_service.resolve_user_ns(access_q.access_token.as_deref(), access_q.secure_key.as_deref()).await.map_err(|_| AppError::BadRequest("NEED_LOGIN".to_string()))?;
...
pub async fn save_book_group_id(State(state): State<AppState>, Query(access_q): Query<AccessTokenQuery>, Json(param): Json<SaveBookGroupIdParam>) -> Result<Json<ApiResponse<Value>>, AppError> {
    let user_ns = state.user_service.resolve_user_ns(access_q.access_token.as_deref(), access_q.secure_key.as_deref()).await.map_err(|_| AppError::BadRequest("NEED_LOGIN".to_string()))?;
...
pub async fn add_book_group_multi(State(state): State<AppState>, Query(access_q): Query<AccessTokenQuery>, Json(param): Json<MultiBookGroupParam>) -> Result<Json<ApiResponse<Value>>, AppError> {
    let user_ns = state.user_service.resolve_user_ns(access_q.access_token.as_deref(), access_q.secure_key.as_deref()).await.map_err(|_| AppError::BadRequest("NEED_LOGIN".to_string()))?;
...
pub async fn remove_book_group_multi(State(state): State<AppState>, Query(access_q): Query<AccessTokenQuery>, Json(param): Json<MultiBookGroupParam>) -> Result<Json<ApiResponse<Value>>, AppError> {
    let user_ns = state.user_service.resolve_user_ns(access_q.access_token.as_deref(), access_q.secure_key.as_deref()).await.map_err(|_| AppError::BadRequest("NEED_LOGIN".to_string()))?;
    let urls = param.book_urls.unwrap_or_default();
    let gid = param.group_id.unwrap_or(0);
